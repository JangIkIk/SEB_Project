 ...
 */
  it('원시 자료형은 값 자체에 대한 변경이 불가능(immutable)합니다.', function () {
    let name = 'codestates';
    // 해석하자면 원시자료형은 재할당은 가능하지만. 메서드,함수등로 변경한값을 return하여도 원래의 값을 가지고있다.
    // 하지만 원시 자료형이 아닌 function,Object,Array는 method,function을통한 return값은 원래의 값을 변경시킨다.
  ...
  it('원시 자료형 또는 원시 자료형의 데이터를 함수의 전달인자로 전달할 경우, 값 자체의 복사가 일어납니다.', function () {
    ...
    // afterTenYears 은 리턴문이 없을뿐아니라
    // 원래의 값을 바꾸지는 못함 변경된값을 담을경우는 새로운 변수선언이 필요!!
  ...
  */
  ...
  it('참조 자료형을 변수에 할당할 경우, 데이터의 주소가 저장됩니다.', function () {
    ...
    */
    ...
    const ages = [22, 23, 27];
    allowedToDrink = ages;
    // 아래코드는 ages와 같은 주소를 참조 하고 있기 떄문에 true!!
    expect(allowedToDrink === ages).to.equal(true);
    // 아래코드는 값은 맞지만 엄격하게 따질경우 참조하는 주소가(stack/heap) 다르다!
