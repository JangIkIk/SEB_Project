...
function createTreeView(menu, currentNode) {
  ...
  for (let i = 0; i < menu.length; i++) {
    // type, name : li -> input, span
    // children   : [] or undefined
    const title = document.createElement("li");
    const check = document.createElement("inp  ut");
    const content = document.createElement("span");
    const ul = document.createElement("ul");

    if (menu[i].children) {
      check.setAttribute("type", "checkbox") // input type 변경
      content.append(menu[i].name) // span안에 menu객체의 name
      title.append(check) // li 태그안에 input
      title.append(content) // li 태그안에 span       
      title.append(ul); // li 태그안에 ul
     
      createTreeView(menu[i].childre n, ul);
    } else {
      title.append(menu[i].name)
      ul.append(title)
      

    }
    currentNode.append(title) // root에 자식으로 title 추가 (input, span)

  }
  
  // return createTreeView(menu.length - 1, currentNode)  
