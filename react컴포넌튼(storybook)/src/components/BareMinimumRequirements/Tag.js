...
export const Tag = () => {
  ...
  const removeTags = (indexToRemove) => {
    ...
    let result = tags.filter((ele)=>{
      if(ele !== indexToRemove){
        return ele;}
      })
    setTags(result);
  ...
  const addTags = (event) => {
    ...
    // console.log("엔터시에만 실행!")
    // console.log(event)
    // console.log(event.target.value);
    // console.log(tags);
    const result = event.target.value;

    if(!tags.includes(result) && result !== undefined){
      setTags([...tags, result])
        ...
        <ul id="tags">
          {tags.map((tag, index) => (
            <li key={index} className="tag">
              ...
              <span className="tag-close-icon" onClick={()=>{removeTags(tag)}}>x
