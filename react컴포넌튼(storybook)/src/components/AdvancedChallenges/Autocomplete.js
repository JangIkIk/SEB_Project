...
export const Autocomplete = () => {
  ...
  const handleKeyUp = (event) => {
    if(event.key === "ArrowDown" && sel < options.length -1){
      setSel(sel + 1);
    }
    if(event.key === "ArrowUp" && sel > -1){
      setSel(sel - 1);
    }
    if(event.key === "Enter"){
      handleDropDownClick(options[sel]);
      setSel(-1);
    }
  }
  ...
  return (
    <div className='autocomplete-wrapper'>
     ...
     {hasText ?  <DropDown
       options={options}
       handleComboBox={handleDropDownClick} sel={sel}
       /> : null}
    </div>
  );
};
...
export const DropDown = ({ options, handleComboBox }) => {
  return (
    <DropDownContainer>
      ...
      {options.map((el) => <li onClick={()=>handleComboBox(el)}>{el}</li>)}
