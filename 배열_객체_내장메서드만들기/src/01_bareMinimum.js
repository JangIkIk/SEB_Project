 ...
 *  배열 arr을 입력받을 경우, iteratee(ele, idx, arr) // 요소,인덱스,배열
 *  객체 obj를 입력받을 경우, iteratee(val, key, obj) // 값 , 키 , 오브젝트
...
_.each = function (collection, iteratee) {
  // TODO: 여기에 코드를 작성합니다.
  // iterator(반복시키는), iteratee(반복을 당하는) 콜백함수
  // collection === 배열이나,객체
...
_.indexOf = function (arr, target) {
  ...
  //  arr =>  배열?
  // target => 배열의 요소인경우? 
 // _.indexOf = function ([10, 20, 30, 40, 50],40)

  let result = -1;

  _.each(arr, function (item, index) {
    /* _.each([10, 20, 30, 40, 50],function (요소,인덱스){
        if(40 === 40 && result = -1){
          result = 3;
        }
    }  )
...
_.filter = function (arr, test) {
  // TODO: 여기에 코드를 작성합니다.
  // _.each함수 사용
  // _.each함수를 통해 배열,객체를 걸러낸후 test함수를 호출한다. => test함수는 정의되지 않았음
 /* 배열의 내장함수인 filter() 기능정의 ~
    이 filter()메서드 는 제공된 함수로 구현된 테스트를 통과하는 지정된 배열의 요소만
    필터링하여 지정된 배열의 일부에 대한 얕은 복사본 을 만듭니다.
    function isBigEnough(value) {
    return value >= 10;
      
    const filtered = [12, 5, 8, 130, 44].filter(isBigEnough);
    이와같은 예시의 경우 isBigEnough의 함수를 통해 10과 같거나 큰 값들만 filtered에 할당됨!!
*/
    let result = [];
...
_.uniq = function (arr) {
  // TODO: 여기에 코드를 작성합니다.
  // arr 에 [1, 2, 2, 3, 4, 4]; 중  중복이 제거된 값을 result 에넣는다 => [1,2,3,4]

  // _.uniq = function (arr) {
  //   let result = [];
  //  _.filter(arr,function(item){ //filter 조검이 참이면 리턴
  //   if (-1 === _.indexOf(arr,item)) { //indexof arr에 item이 없을 때
  //       return item;
  //   } 
  //   result.push(item);
  //  }) 
...
_.reduce = function (arr, iteratee, initVal) {
  ...
  // 누적값 : acc
  // 현재값 : cur
  // initVal이 있다면 acc === initVal
  // initVal이 없다면 acc === arr[0];

  let acc = initVal; // 초기값이 있으면 값 ,초기값어 없으면undefined
  _.each(arr,(ele,idx,arr)=>{
    if(acc === undefined){
      acc = arr[0];
    } else{
